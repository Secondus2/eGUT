...
public class Collision
{
			...
			double pullDistance)
	{
		...
		for ( Surface a : allA )
		{
			for ( Surface b : allB )
			{ 
				this.collision( a, agent, b, neighbour, this._variables );
			}
		}
	}
			...
			Ball sphere, OrientedCuboid otherSurface, CollisionVariables var)
	{
		...
		for (int i = 0; i < normal.length; i++)
		{
			if (normal[i] == 1.0)
			{
				var.setDistance(sphereCentre[i] - apicalFace[0].getPosition()[i]);
			}
			else if (normal[i] == -1.0)
			{
				var.setDistance(apicalFace[0].getPosition()[i] - sphereCentre[i]);
			}
		}
			...
			OrientedCuboid cuboid, CollisionVariables var)
	{
		...
		for (int i = 0; i < coordinates.length; i++)
		{
			...
			double cornerDist = Double.valueOf(var.getDistance());
			...
			Rod rod, OrientedCuboid otherSurface, CollisionVariables var)
	{
		...
		if (distanceA < distanceB)
		{
			...
			var.t = 1.0;
			var.s = 0.0;
			var.setDistance(distanceA - rod.getRadius());
			if (extend)
		...
		else 
		{
			...
			var.t = 0.0;
			var.s = 1.0;
			var.setDistance(distanceB - rod.getRadius());
			if (extend)
				var.radiusEffective = rod.getRadius();
			return var;
		}
